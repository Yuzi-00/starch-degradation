...
filename1 <- "C:/Users/WAN333/Documents/Thesis/Experiments/2019-10/20191028_Hydrolysis Plate 10/raw data/20191028_DNS plate10-t"
filename2 <- 'min.xlsx'
...
data_plate10 <- read_excel(paste0(filename1,'0',filename2), range = "A15:M23") 

# add two columns: plate number and time point for the 1st spreadsheet

data_plate10 <- mutate(data_plate10, plate = 10, time = 0)
...
for (fltime in filetime) {
  ...
  temp_data <- mutate(temp_data,  plate = 10, time = as.numeric(fltime))
  # add two columns: plate number and time point 
  data_plate10 <- bind_rows(data_plate10, temp_data) # bind them by raws
}

# modify the default name for the first col & raw

data_plate10 <- rename(data_plate10, raw = ...1) 

# transfer the data into two columns

data_plate10 <- gather(data_plate10, col, OD, -raw, -plate, -time) 
