...
names(hydro_data)
hydro_data$residuals

hydro_data$fitted = fitted(model)

ggplot(hydro_data, aes(x = fitted, y = residuals, color = Sample %in% "C-")) +
  geom_point()

which.min(hydro_data$residuals)
hydro_data$Sample[277]
######################################## plot ######################################################
...
## check the residuals
 ...
 plot(model_92) # it seems that there is a pattern there ?
 
 ############################# calculate fitted hydrolysis extent ###########################################

 # read in the estimated parameters dataset 
 
 parameters <- read_csv("analysis/fitted_parameters.csv") 
 
 # read in the other dataset in order to get the Sample and the Time columns
 
 data_6P <- read_csv("data/tidydata/data_6P_cal_3nd.csv") %>% 
   select(Sample, Time)
 
 # creat a vector that contains time points from 0 to 1800
 
 Time <- c(0:1800)
 
map
 
 # combine these two dataset above together by Sample names
 
 new_data <- left_join(data_6P, parameters)
