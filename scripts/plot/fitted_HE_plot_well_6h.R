...
pdf(file = "figures/fit_HE_6h_well_with_T0.pdf") # creating a pdf file and senting all the plot below to this file
for(i in unique(total$Well)){ # i stands for each item within this dataset
  ...
  model_x <- total %>%
    filter(Time != 1440 & Time != 1800) %>% # remove the last two time points 
    filter(Well == i) %>%
