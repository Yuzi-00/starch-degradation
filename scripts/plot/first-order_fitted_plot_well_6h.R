...
pdf(file = "figures/first-order_6h_fit_HE_well_with_T0.pdf") # creating a pdf file and senting all the plot below to this file
for(i in unique(total$Well)){ # i stands for each item within this dataset
  ...
  model_x <- total %>%
    filter(Well == i) %>%
    nls(formula = HE ~ Xinf*(1-exp(-k*Time)), 
        data = . ,
  ...
  model_y <- total %>%
    ...
    filter(Well == i) %>%
    nls(formula = HE ~ Xinf*(1-exp(-k*Time)), # using the Weibull function 
                 data = . ,
                 algorithm = "port", # add this if setting the constrains 
