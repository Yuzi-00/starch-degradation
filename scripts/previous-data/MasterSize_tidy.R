...
master_size <- df %>% 
  select("Sample Name", "Specific surface area", "d (0.1)", "d (0.5)", "d (0.9)") %>%  # select just these useful columns
  rename(Sample = "Sample Name", SSA = "Specific surface area", d1 = "d (0.1)", d5 = "d (0.5)", d9 = "d (0.9)") %>% 
# change the column name to be consistant with the other dataset
  group_by(Sample) %>% # group by sample
  summarise(SSA = mean(SSA), D1 = mean(d1, na.rm = TRUE), D5 = mean(d5, na.rm = TRUE), D9 = mean(d9, na.rm = TRUE)) # calculate the average diameters
