...
# calculate the mean, sd and coefficient of variance of the hydrolysis extent by ID and by Time points

mean_by_ID <- cal_6P_3nd %>% 
  filter(!(is.na(ID))) %>% 
  group_by(ID, Time) %>% 
...
write_csv(mean_by_ID, "analysis/cal_by_ID.csv")

# plotting

ggplot(data = mean_by_ID, # plotting by Time and the mean values of HE
       aes(x = Time, 
           y = HE,
           group = Sample,
           color = status)) +
  geom_point(size = 1.6, alpha = 0.5) + # add the transparency
